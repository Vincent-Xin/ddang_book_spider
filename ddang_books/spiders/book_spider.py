    ...
    def parse(self, response):
        ...
        if categories:
            for category in categories:
                first_href = category.xpath('./@href').get()
            ...
            if book_list:
                brandcrumbs = response.xpath('//div[@id="breadcrumb"]/div[@class="crumbs_fb_left"]/div[@class="select_frame"]/a/text()').getall()
                categories = [brandcrumb.strip() for brandcrumb in brandcrumbs[1:]]
                for i in range(len(book_list)):
                    ...
                    img_url = response.css('div#search_nature_rg>ul>li:nth-child(' + i + ')>a>img::attr(data-original)').getall()
                    if not img_url:
                        img_url = response.css('div#search_nature_rg>ul>li:nth-child(' + i + ')>a>img::attr(src)').getall()
                    item_loader.add_value('image_url', [img_url])
                    author = response.css('div#search_nature_rg>ul>li:nth-child(' + i + ')>p.search_book_author>span:nth-child(''1)>a::attr(title)').get(default="缺")
                    item_loader.add_value('author', author)
                    pub_time = response.css('div#search_nature_rg>ul>li:nth-child(' + i + ')>p.search_book_author>span:nth-child(''2)::text').get(default='2020-01-01')
                    item_loader.add_value('pub_time', pub_time)
                    publisher = response.css('div#search_nature_rg>ul>li:nth-child(' + i + ')>p.search_book_author>span:nth-child(''3)>a::attr(title)').get(default="缺")
                    ...
                    page_num_str = response.css('div#go_sort div.data>:nth-child(3)::text').get().strip()
                    page_num = int(re.match('/(\d+)', page_num_str).group(1))
                    if page_num > 1:
                        for i in range(2, page_num + 1):
                            next_page_url = f'http://category.dangdang.com/pg{str(i)}-{response.url[-24:]}'
